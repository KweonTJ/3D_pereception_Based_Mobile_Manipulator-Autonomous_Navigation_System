...
- transformer는 같은 raw trajectory target이 반복 발행되면 최초 수신 시점의 joint3 reference를 재사용한다. 이전 방식처럼 매 반복마다 현재 joint3를 새 reference로 잡으면 `-0.12 rad` 제한이 매번 누적되어 joint3가 계단식으로 계속 펴지는 문제가 생긴다.
...
이 단계에서 `/mp_control/status`에는 `rpy_err=(roll, pitch, yaw)`, `roll_ref=stay_roll`, `rpy_ready`, `rpy_frame`, forward advance 진행 상태가 표시된다. 실제 파지 순서는 `베이스 정지 -> joint pregrasp 자세 생성 -> joint2/joint3 중심 추가 전개 -> joint4 보조 보정 -> 그리퍼 close`다. close는 joint4 완료가 아니라 `advanced_x`, `eef_forward_distance_m`, `eef_forward_min_advance_before_close_m`, joint3 전개 진행률을 중심으로 판단한다. `min_advance`는 close 하한이고, 그 자체로 팔 전개 완료를 의미하지 않는다.
