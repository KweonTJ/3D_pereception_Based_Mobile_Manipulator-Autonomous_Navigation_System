...
- `0.20 m`: 컬러 삼각 측량 기반 베이스 접근 목표 거리다. 이 거리 이후 팔 파지 단계로 넘어간다. `arm_start_max_object_x_m`도 같은 `0.20 m`로 맞춰 베이스가 충분히 가까워지기 전에는 팔 단계로 넘어가지 않게 한다. 단, 전면 bbox가 close-range handoff 크기를 넘고 EEF bbox가 fresh하면 실제 시각적으로 가까운 상태로 보고, 거리 추정값이 `0.20 m`보다 약간 크게 나와도 joint pregrasp를 시작한다. `0.19 m` 실험에서는 로봇과 물체가 너무 가까워져 팔이 뻗기 전에 EEF 시야와 전개 공간이 부족했기 때문에, 현재 기준은 `0.20 m`다. 전면 bbox 크기 기반 3D point 변환이 TF 문제로 실패해도, 기존 depth reference가 남아 있으면 bbox 크기 close 판정만으로도 pregrasp를 허용한다.
...
- `close_on_front_bbox_shrink / front_bbox_close_area_ratio`: EEF fixed-pose forward 시작 시점의 전면 bbox 면적을 기준으로 저장하고, 이후 전면 bbox 면적이 그 기준의 `0.55` 이하로 줄면 그리퍼 close를 시작한다. 베이스 정지 거리를 `0.20 m`로 맞춰 forward 시작 bbox가 `0.21 m` 설정보다 크고 `0.19 m` 설정보다 덜 극단적이므로, close 기준도 중간값으로 둔다.
...
그리퍼 close 자체는 EEF forward 거리만으로 기다리지 않는다. EEF forward 시작 시점의 전면 bbox와 EEF bbox 면적을 각각 저장하고, 현재 전면 bbox 면적이 `front_bbox_close_area_ratio` 이하이거나 현재 EEF bbox 면적이 `eef_bbox_close_area_ratio` 이하가 되면 물체가 그리퍼 안쪽으로 충분히 들어왔다고 보고 close 명령을 보낸다. 현재 실제 설정은 둘 다 `0.55`이다. 베이스가 `0.20 m`까지 접근하면서 forward 시작 bbox가 커지므로, 이전 `0.60`보다 조금 더 작은 ratio까지 기다려 premature close를 줄이되 `0.19 m` 실험의 `0.50`보다는 빨리 close할 수 있게 한다.
