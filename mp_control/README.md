...
- `eef_forward_roll_joint*_weight`: 추가 전진에서 그리퍼 roll proxy를 계산하는 조인트 가중치다. 실제 리더에서 joint3이 음수 방향으로 펴질 때 joint4가 양수 방향으로 움직이면 그리퍼와 EEF 카메라가 공중을 보는 문제가 확인되었다. 따라서 현재 실제 설정은 `eef_forward_roll_joint3_weight: -1.0`, `eef_forward_roll_joint4_weight: 1.0`으로 두어 joint3이 음수 방향으로 펴질 때 joint4도 음수 방향으로 보상되게 한다.
